...
def invalid(stdscr):
    stdscr.addstr(13, 0, "Enter your choice: ", curses.color_pair(1))
    stdscr.refresh()
    time.sleep(.1)
...
def add_student(stdscr, s_list, c_list, m_list):
    new_s = Student()
    new_s.set_sid(stdscr)
    new_s.set_sname(stdscr)
    new_s.set_sdob(stdscr)
...
def add_n_student(stdscr, s_list, c_list, m_list):
    stdscr.refresh()
    while True:
        curses_n_item(stdscr, 0)
        try:
            times = int(curses_time_item(0))
        except ValueError:
            times = -1
        if times == 0:
            return s_list, m_list
        if times < 0:
            ...
            for i in range(times):
                curses_ordinal(stdscr, ordinal(i + 1), 0)
                s_list, m_list = add_student(stdscr, s_list, c_list, m_list)
            break
    return s_list, m_list


def curses_n_item(stdscr, mode):
    label = [
        "Enter number of new students: ",
    ]
    blank(stdscr, 1)
    stdscr.addstr(3, 0, "[0] Exit")
    stdscr.addstr(13, 0, label[mode])
    stdscr.refresh()


def curses_time_item(mode):
    label = [
        "Enter number of new students: ",
    ]
    win = curses.newwin(1, 30, 13, len(label[mode]))
    box = Textbox(win)
    curses.curs_set(1)
    box.edit()
    curses.curs_set(0)
...
class Student:
    ...
    def set_sname(self, stdscr):
        sname = input("Enter student name: ")
        if not sname:
            sname = self.__default_sname
        self.__sname = sname

    def set_sdob(self, stdscr):
        while True:
            try:
